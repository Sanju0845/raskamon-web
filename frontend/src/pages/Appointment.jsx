  ...
  const getAvailableSlots = async (date = new Date()) => {
    ...
    try {
      const availabilityResponse = await axios.get(
        `${backendUrl}/api/doctor/availability/${docInfo._id}`,
      );
      if (availabilityResponse.data.success) {
        doctorAvailability = availabilityResponse.data.availability;
      }
    } catch (error) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const scheduleStartTime = dayAvailability?.startTime || "00:00";
      const scheduleEndTime = dayAvailability?.endTime || "23:30";
          ...
          <section className="py-16">
            <div className="max-w-7xl mx-auto px-1 sm:px-6 lg:px-8">
              <div className="grid lg:grid-cols-3 gap-8">
                ...
                >
                  <div className="bg-white/18 backdrop-blur-sm rounded-3xl shadow-xl p-4  border border-white/20">
                    ...
                    <div className="mb-8 flex flex-wrap gap-8">
                      ...
                      <div className="flex flex-col gap-2">
                        <h3 className="text-xl font-semibold text-gray-800  pb-1">
                          Specializations
                        </h3>
                          ...
                          {docInfo.languageSpoken &&
                            docInfo.languageSpoken
                              ...
                              .map((lang, index) => (
                                <span
                                  key={index}
                                  className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-6 py-3 rounded-full text-sm font-medium shadow-lg"
                                >
                                  {lang}
                                </span>
                              ))}
              ...
              >
                ...
                <div className="mb-12">
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                    ...
                    {/* Date pills - only show days with available slots */}
                    {docSlots.map(
                      (item, index) =>
                        item.length > 0 &&
                        item[0]?.datetime && (

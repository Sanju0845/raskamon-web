...
const Resources = () => {
  ...
  const fetchBlogPosts = async (page) => {
    try {
      ...
      const { data } = await axios.get(
        `${backendUrl}/api/blog-posts/approved?page=${page}&limit=${limit}`
      );
  ...
  useEffect(() => {
    ...
    featuredPostsRef.current = featuredPosts; // store a stable reference
  }, [featuredPosts]);

  useEffect(() => {
    if (featuredPostsRef.current.length === 0) return;

    const timer = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % featuredPostsRef.current.length);
    }, 5000);
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-center p-8 md:p-12">
                        <div className="text-white">
                          <div className="inline-block bg-purple-600/80 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-medium mb-4">
                          ...
                          <div className="flex items-center 0 gap-4">
                            <button className="text-gray-600 rounded-3xl shadow-md bg-white px-4 py-2">
                              Read More
                            </button>
        ...
        pagination.hasPrev
          ...
          : "bg-gray-300 text-gray-500 cursor-not-allowed"
            ...
            <span className=" font-medium">
              Page <span className="font-bold">{pagination.currentPage}</span>{" "}
